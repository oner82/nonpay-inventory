...
- `window.create<Feature>Module = (context) => ({ render..., bind... })` — e.g. `createDashboardModule`, `createReceiptsModule`, `createUsageEntryModule`, `createRoomCloseModule`, `createHistoryModule`, `createProductsModule`, `createImplantsModule`, `createSettingsModule`, `createDepartmentsModule`, `createImplantVendorsModule`, `createUsageRulesModule`, `createBackupResetModule`.
...
- **비급여 = 방 마감(방 단위 보충) 집계 (v2.1.0):** 각 방의 비급여 상자는 정수(par 10개)로 운영되고, 마감 스크럽이 "방 마감" 화면에서 **채운 수량 = 그날 그 방의 사용량**으로 입력한다 (`state.roomRefills`: {id, date, room, items:[{productId, qty}]}, 같은 날+방은 덮어쓰기). `productMovementCounts`와 아침점검이 roomRefills를 사용량으로 합산해 기존 재고 수식이 유지된다. **사용입력(케이스)에서는 비급여가 목록·검색·추천에 나오지 않는다** — 인체조직/임플란트만 케이스 단위. 총 재고는 보관소+방 배치분 합산 개념.
