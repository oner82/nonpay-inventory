...
const renderDashboard = () => {
  const lowProducts = state.products
    .filter((item) => num(item.stock) <= num(item.warningStock))
    .sort((a, b) => num(a.stock) - num(b.stock) || alphaFirstCompare(a.name, b.name));
  ...
  const pendingLandingLines = landingUsageLines(false);
  const openPendingUsageItems = pendingUsagesOpen();
  const totalStock = state.products.reduce((sum, item) => sum + num(item.stock), 0);

  const todayCategoryCounts = PRODUCT_CATEGORIES.map((category) => ({
    category,
    count: todayUsages.reduce((sum, usage) => sum + (usage.productIds || []).filter((id) => productCategory(productById(id)?.category) === category).length, 0)
  }));
  ...
  const topUsedProducts = Array.from(productUseMap.entries())
    .map(([productId, count]) => ({ product: productById(productId), count }))
    .filter((item) => item.product)
    .sort((a, b) => b.count - a.count || alphaFirstCompare(a.product.name, b.product.name))
    .slice(0, 8);
  ...
  const pendingByCompany = Array.from(pendingLandingLines.reduce((map, line) => {
    ...
    current.lines += 1;
    current.qty += Math.max(1, num(line.qty));
    current.products.add(line.product.name);
    map.set(company, current);
    return map;
  }, new Map()).values()).sort((a, b) => b.lines - a.lines || alphaFirstCompare(a.company, b.company));
    ...
    <section class="dashboard-shell">
      ...
      <div class="dashboard-content">
              ...
              <details class="summary-row" open>
                <summary>
                  ...
                  <div class="summary-metrics">
                    ${todayCategoryCounts.map((item) => `<div class="metric"><strong>${item.count}</strong><span>${escapeHtml(productCategoryLabel(item.category))}</span></div>`).join("")}
                  </div>
                </summary>
              </details>
            </div>
            ${todayUsageBrief.length ? todayUsageBrief.map((item) => usageItem(item, { showDelete: false })).join("") : `<div class="empty">오늘 입력된 사용내역이 없습니다.</div>`}
            <div class="actions">
            ...
            ${pendingByCompany.length ? pendingByCompany.slice(0, 6).map((item) => `
              <div class="item landing-line pending">
                <div class="compact-line">
                  <div class="compact-main">${escapeHtml(item.company)}</div>
            ...
            ${topUsedProducts.length ? topUsedProducts.map((item, index) => `
              <div class="item">
                <div class="item-title">
                  <span>${index + 1}. ${escapeHtml(item.product.name)}</span>
                  <span class="pill">${item.count}개</span>
                </div>
                <div class="meta">
                  <span>${escapeHtml(productCategoryLabel(item.product.category))}${item.product.company ? ` · ${escapeHtml(item.product.company)}` : ""}${item.product.subcategory ? ` · ${escapeHtml(item.product.subcategory)}` : ""}</span>
                  <span>현재고 ${num(item.product.stock)} · 경고수량 ${num(item.product.warningStock)}</span>
                </div>
              </div>
        ...
        <div class="dashboard-right">
          <div class="card">
            <h2>재고 부족 우선 확인</h2>
            ${lowPriorityProducts.length ? lowPriorityProducts.map(lowProductItem).join("") : `<div class="empty">표시할 재고 부족 품목이 없습니다.</div>`}
            ${hiddenLowProducts.length ? `<details class="item"><summary><span>감춰진 경고 재고</span><span class="pill">${hiddenLowProducts.length}</span></summary><div class="details-body">${hiddenLowProducts.map((item) => lowProductItem(item, true)).join("")}</div></details>` : ""}
            <div class="actions">
          ...
          <div class="card">
            ...
            <div class="summary-table">
              ${PRODUCT_CATEGORIES.map((category) => {
                const items = state.products.filter((item) => productCategory(item.category) === category);
                const stock = items.reduce((sum, item) => sum + num(item.stock), 0);
                const low = items.filter((item) => num(item.stock) <= num(item.warningStock)).length;
                return `
                  <details class="summary-row ${category === "비급여" ? "nonpay" : ""}" open>
                    <summary>
                      <div class="summary-headline">
                        <span class="summary-name">${escapeHtml(productCategoryLabel(category))}</span>
                      ...
                      <div class="summary-metrics">
                        ...
                        <div class="metric"><strong>${items.reduce((sum, item) => sum + num(item.warningStock), 0)}</strong><span>경고합</span></div>
                      </div>
                    </summary>
                  </details>
                `;
          ...
          <div class="card">
            <h2>최근 사용 내역</h2>
            ${state.usages.slice().reverse().slice(0, 4).map((item) => usageItem(item, { showDelete: false })).join("") || `<div class="empty">사용내역이 없습니다.</div>`}
          </div>
        </div>
      </div>
    </section>
...
const statCard = (label, value, icon) => `
  <div class="card stat" data-icon="${escapeHtml(icon)}">
    <strong>${value}</strong>
    <span>${label}</span>
  </div>
`;
...
const renderReceipts = () => {
  ...
  const receiptView = allowedReceiptViews.includes(currentReceiptView) ? currentReceiptView : allowedReceiptViews[0];
  return `
  <section class="grid">
...
const bindReceipts = () => {
  app.querySelectorAll("[data-receipt-view]").forEach((button) => {
    button.addEventListener("click", () => {
      currentReceiptView = button.dataset.receiptView;
      render();
    });
  });
      ...
      <div class="row three history-filter-grid">
        ...
        <div>
          ...
          <datalist id="historyProductList">
            ${state.products.slice().sort(byName).map((product) => `<option value="${escapeHtml(product.name)}"></option>`).join("")}
          </datalist>
        </div>
      </div>
      <div class="actions">
    ...
    <div class="card">
      <details class="item">
        ...
        <div class="details-body">
          ...
          <div id="historyPatientList">${state.usages.slice().reverse().map(usageItem).join("") || `<div class="empty">사용내역이 없습니다.</div>`}</div>
        </div>
      </details>
    </div>
  </section>
...
const bindHistory = () => {
  ...
  const updateHistory = () => {
    ...
    patientList.innerHTML = usages.map(usageItem).join("") || `<div class="empty">사용내역이 없습니다.</div>`;
  ...
  const bindHistoryDeleteButtons = () => {
    ...
    patientList.addEventListener("click", async (event) => {
      ...
      if (button.dataset.editUsage) {
        pendingEditUsageId = button.dataset.editUsage;
        currentView = "edit";
        render();
        return;
      }
      if (!button.dataset.deleteUsage) return;
      await deleteUsageRecord(button.dataset.deleteUsage, { onSuccess: updateHistory });
    });
  };
  bindHistoryDeleteButtons();
  bindHistoryExports();
